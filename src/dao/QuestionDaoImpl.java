...
{
	private static final String QUESTIONACTIVE	  					= "active";
	
    private static final String SQL_SELECT        					= "SELECT idQuestion, idQuestionnaire, intitule, ordre, actif FROM question ORDER BY idQuestionnaire";
    ...
    private static final String SQL_UPDATE        					= "UPDATE question SET idQuestionnaire = ?, intitule = ?, ordre = ?, actif = ? WHERE idQuestion = ?";
    private static final String SQL_DELETE_BY_ID 					= "DELETE FROM question WHERE idQuestion = ?";
    private static final String SQL_SELECT_SURVEY_BY_ID				= "SELECT id, sujet, actif, createdAt FROM questionnaire WHERE id = ?";
    ...
    public void create( Question question ) throws DAOException 
    {
        ...
        try 
        {
        	...
        	if(sStatus.equals( QUESTIONACTIVE )) 
        	{
        		bActive = true;
        	}
    ...
    public void modify( Question question ) throws DAOException 
    {
        ...
        try 
        {
            ...
            preparedStatement = PreparedRequestInitialization( connection, SQL_UPDATE, true, question.getSurvey().getId(),
            		question.getText(), question.getOrder(), bActive, question.getId());
            int status = preparedStatement.executeUpdate();
            if ( status == 0 ) 
            {
                throw new DAOException( "Échec de la création de la question, aucune ligne ajoutée dans la table." );
    ...
    public List<Question> lister() throws DAOException 
    {
        ...
        {
            connection = daoFactory.getConnection();
            preparedStatement = PreparedRequestInitialization( connection, SQL_SELECT, false );
            resultSet = preparedStatement.executeQuery();
            while ( resultSet.next() ) 
            {
                questions.add( map( resultSet ) );
    ...
    private Question map( ResultSet resultSet ) throws SQLException 
    {
        ...
        Survey survey = findSurvey(SQL_SELECT_SURVEY_BY_ID, resultSet.getLong("idQuestionnaire"));
